 ...
 void api_db_open()
{
	...
	db->Record = malloc(sizeof(u8 *) * 1048575);
	if (db->Record == NULL)
	{
		db->Record = malloc(sizeof(u8 *) * 65535);
		if (db->Record == NULL)
		{
			tNativeFunction_PushReturnAsNull();
			return;
...
/*=========================================================================
* FUNCTION:     db_set_numberic
* TYPE:          function 
* OVERVIEW:      用指定的字符串 设置指定的 N类型 的字段值 

*   parameters:  字段名,数据库指针,值
*   returns:     有效记录个数 类型u32 
*=======================================================================*/
void db_set_numberic(u8 *param, Database *db, u8* value)
{
	int i;
	s16 j = 1;
	int k;
...
* TYPE:          function 
* OVERVIEW:      用指定的f64 设置指定的 N类型 的字段值 
...
/*=========================================================================
* FUNCTION:     db_set_boolean
* TYPE:          function 
* OVERVIEW:      设置指定的 L类型 的字段值 

*   parameters:  字段名,数据库指针,值
*   returns:     有效记录个数 类型u32 
*=======================================================================*/
void db_set_boolean(u8 *param, Database *db, s32 value)
{
	int i;
	s16 j = 1;	
...
//	reFiledValue.value.str_val = NULL;
	...
	if (iget)
	{
		...
		case 'N':
			...
			break;
		case 'D':
			reFiledValue.value.date_val = malloc (11);
		...
		case 'F':
			...
			break;
		case 'L':
			if (db->Record[col-1][j] == 'T')
				reFiledValue.value.int_val = 1;
			else
				reFiledValue.value.int_val = 0;
			break;
		default:
			break;
		}
...
void api_db_locate()
{
	...
	{ 
		...
		if(apihelper_db_isinvalide(par_FdValue))
		{
			printf("语法错误！");
			break;
		}
		...
		{
			api_db_ext_Database->RecordNo = i;
			par_Boolean = true;
			break;
		}
		else
	...
	if(i == api_db_ext_Database->RecordCount)
	{
		par_Boolean = false;
	}
...
void api_db_count(void)
{
	...
	{ 
		api_db_ext_Index = i;
		api_db_ext_Length = 0;
		if(apihelper_db_isemptycondition()&&!apihelper_db_isdeleted(i))
		{
			par_Count ++;
		}
		else
...
void apihelper_db_getword()
{
	...
	/*39是单引号的ASKII值*/
	if(api_db_ext_Letter != 39)
	{
		if(api_db_ext_Letter == 'a' || api_db_ext_Letter == 'o')
		{
			...
			{
				apihelper_db_getfield();
				return;
			}
		}
		else
		if(apihelper_db_ischletter()||apihelper_db_isletter())
		{
			apihelper_db_getfield();
			return;
		}
	...
	if(api_db_ext_Letter == 39)
	{
		api_db_ext_Operant[0] = 'f';
		//apihelper_db_getstring();
	}
}

void apihelper_db_getfield()
{
	if(apihelper_db_ischletter())
	{
		...
		while(apihelper_db_ischletter())
		{
			...
			while(apihelper_db_isletter()|apihelper_db_isdigit())
			{
				...
				{
					api_db_ext_Letter = toupper(api_db_ext_Letter);   
				}
				apihelper_db_concat();
				apihelper_db_getchar();
			}
		...
		apihelper_db_retract();
		if(apihelper_db_isfieldname(api_db_ext_StrToken))
		{
			api_db_ext_Operant[0] = 's';
		}
		else
		{
		//	printf("找不到列%s",api_db_ext_StrToken);
		//	printf("\n");
			//exit(1);
		}
	}
	...
	{
		/*获取字段名，还没有考虑汉字*/
		if(apihelper_db_isletter()||apihelper_db_isdigit())
		{
			api_db_ext_Letter = toupper(api_db_ext_Letter);
...
dbFieldValue apihelper_db_getstring()
{
	int par_StrLength;
	...
	apihelper_db_getchar();
	apihelper_db_getblank();
	while(api_db_ext_Letter != 39&&api_db_ext_Length < strlen(api_db_ext_Condition))
	{
		apihelper_db_concat();
		apihelper_db_getchar();

		while(apihelper_db_isdigit()&&api_db_ext_Length < strlen(api_db_ext_Condition))
		{
			apihelper_db_concat();
			apihelper_db_getchar();
			if(api_db_ext_Letter == '/')
...
dbFieldValue apihelper_db_getfigure()
{
	int i;
	int par_StrLength;
	dbFieldValue par_RtValue;
	int par_HasADoc = false;
	while(apihelper_db_isdigit())
	{
		apihelper_db_concat();
		if(api_db_ext_Length < strlen(api_db_ext_Condition))
		{
...
dbFieldValue apihelper_db_exp(void)
{
	...
	if(apihelper_db_isinvalide(par_LValue))
	{
		return par_LValue;
	}
	while(api_db_ext_Operant[0] == '&'||api_db_ext_Operant[0] == '|')
	{
		if(api_db_ext_Operant[0] == '&')
		{
			...
			if(apihelper_db_isinvalide(par_RValue))
			{
				return par_RValue;
			}
...
dbFieldValue apihelper_db_factor(void)
{
	...
	{
		/*取得常数*/
		par_LValue = apihelper_db_getfigure();

		apihelper_db_reset();
		apihelper_db_getword();
	}
	else
	if(api_db_ext_Operant[0] == 'f')
	{
		par_LValue = apihelper_db_getstring();

		apihelper_db_reset();
		apihelper_db_getword();
	}
	else
	//if(strcmp(api_db_ext_Operant,"ns") == 0)
	{
		/*显示出错信息,退出*/
		par_LValue.dir = 0;
		par_LValue.dot = 0;
		par_LValue.len = 0;
		par_LValue.type = 0;
		par_LValue.value.str_val[0] = 0;
		par_LValue.value.date_val = 0;
		par_LValue.value.int_val = 0;
		par_LValue.value.memo_val = 0;
		par_LValue.value.pstr_val = 0;
		//printf("语法错误!\n");
		//exit(1);
	}
	return par_LValue;
}
int apihelper_db_isinvalide(dbFieldValue fieldValue)
{
	if(fieldValue.dir == 0
		&&fieldValue.dot == 0
		&&fieldValue.len == 0
		&&fieldValue.type == 0
		&&fieldValue.value.str_val[0] ==0
		&&fieldValue.value.date_val == 0
		&&fieldValue.value.int_val == 0
		&&fieldValue.value.memo_val == 0
		&&fieldValue.value.pstr_val == 0)
...
dbFieldValue apihelper_db_getrecord()
{
	...
	while(*(par_Record + par_Temp)== ' '&&par_LBlank < par_RecordLen)
	{
		par_Temp ++;
		par_LBlank++;
	}
	/*为空串*/
	if(par_LBlank == par_RecordLen||*(par_Record) == '\n')
	{
		par_RtValue.type = 'C';
		par_RtValue.len = 0;
		par_RtValue.dir = 'F';
	...
	while(*(par_Record + par_Temp) == ' '&&par_Temp > 0)
	{
		par_RBlank ++;
		par_Temp --;
	}
...
int apihelper_db_isfieldname(u8 *fieldName)
{
...
dbFieldValue apihelper_db_compop(dbFieldValue lop,dbFieldValue rop,u8 *isgreat)
{
	...
	if((*par_LValue < '0'|| *par_LValue > '9')&&strcmp("=",isgreat)==0)
	{
		...
		if(lop.len > rop.len && lop.dir == 'T'&& rop.dir == 'F')
		{
			par_RtValue.value.int_val = false;
			return par_RtValue;
		}
		...
		if(lop.len < rop.len && lop.dir == 'T'&& rop.dir == 'F')
		{
			...
			while(*par_RValue == *par_LValue&&par_Loop < par_MinLength)
			{
				par_RValue ++;
				par_LValue ++;
